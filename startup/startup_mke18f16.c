...
void ResetISR(void) {
    ...
    // Disable and invalidate LMEM code cache BEFORE SystemInit.
    // After J-Link reflash without full POR the cache may hold stale
    // instructions from previous firmware, causing HardFaults in
    // seemingly innocent code (e.g. CLOCK_InitSysPll).
    // PCCCR register at 0xE0082000.
    {
        volatile unsigned int *PCCCR = (volatile unsigned int *)0xE0082000;
        *PCCCR &= ~1U;                         /* clear ENCACHE          */
        *PCCCR |= (1U << 25) | (1U << 24);     /* INVW1 | INVW0          */
        *PCCCR |= (1U << 31);                   /* GO – start invalidate  */
        while (*PCCCR & (1U << 31)) {}          /* wait for completion    */
    }
